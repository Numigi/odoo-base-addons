...
class TestMailFollowers(common.SavepointCase):
    ...
    def test_when_getRecipientData_thenGetAllPartners(self):
        followers = [
            f[0]
            for f in self.mail_followers._get_recipient_data(
                self.main_partner, "email", self.subtype.id
            )
        ]
    ...
    def test_when_getRecipientDataWithContext_thenGetSomePartners(self):
        followers = [
            f[0]
            for f in self.mail_followers.with_context(
                custom_followers=self.partner1
            )._get_recipient_data(self.main_partner, "email", self.subtype.id)
        ]
        assert self.partner1.id in followers
    ...
    def _open_mail_compose_wizard(self):
        wizard_obj = self.env["mail.compose.message"].with_context(
            active_id=self.main_partner.id, active_model="res.partner"
        )
